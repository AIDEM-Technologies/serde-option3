...
pub fn skip_serializing_option3(_args: TokenStream, input: TokenStream) -> TokenStream {
...
fn skip_serializing_none_add_attr_to_field(field: &mut Field) -> Result<(), String> {
    if let Type::Path(path) = &field.ty {
        if is_option3(&path.path) {
            ...
            // Add the option3 attributes
            let attr_tokens = quote!(
                #[serde(skip_serializing_if = "option3::Option3::is_none")]
                #[serde(default = "option3::Option3::default")]
            );
            let attrs = Attribute::parse_outer
                .parse2(attr_tokens)
                .expect("Static attr tokens should not panic");
            field.attrs.extend(attrs);
        } else if is_boxed_option3(&path.path) {
            let has_skip_serializing_if =
                field_has_attribute(&field, "serde", "skip_serializing_if");
            ...
            // Add the boxed option3 attributes
            let attr_tokens = quote!(
                #[serde(skip_serializing_if = "option3::option3_box::is_none")]
...
fn is_option3(path: &Path) -> bool {
    (path.leading_colon.is_none() && path.segments.len() == 1 && path.segments[0].ident == "Option3")
        || (path.segments.len() == 2
            && (path.segments[0].ident == "option3")
            && path.segments[1].ident == "Option3")
}

fn is_boxed_option3(path: &Path) -> bool {
    path.leading_colon.is_none() && path.segments.len() == 1 && path.segments[0].ident == "Box"
        && match &path.segments[0].arguments {
            syn::PathArguments::AngleBracketed(bracketed) => {
                bracketed.args.len() == 1 && match &bracketed.args[0] {
                    syn::GenericArgument::Type(syn::Type::Path(type_path)) => is_option3(&type_path.path),
                    _ => false,
                }
            }
            _ => false,
...
/// ```
///
/// [`serde_as`]: https://docs.rs/serde_with/1.8.0/serde_with/guide/index.html
/// [re-exporting `serde_as`]: https://docs.rs/serde_with/1.8.0/serde_with/guide/serde_as/index.html#re-exporting-serde_as
#[proc_macro_attribute]
...
/// [serde-as-crate]: https://docs.rs/serde_with/1.8.0/serde_with/guide/serde_as/index.html#re-exporting-serde_as
